...
function initHeroImageScroll() {
  ...
  // The viewport Y at which each card should appear centred
  const centreViewportY = (vh - cardH) / 2;

  // scrollY at which each card’s top edge reaches centreViewportY
  const pinScrollYs = imgs.map((img) => {
    const docTop = parseFloat(img.style.top || '0');
    return Math.max(0, docTop - centreViewportY);
  });
