...
/* R39 item 1a (Oscar, 2026-09-04): 120 below the toggle group's bottom —
   supersedes R38 item 3's 80 (same day). */
const GRID_START_GAP_PX = 120;
...
export function initWorkView() {
  ...
  const placeToggle = () => {
    ...
    /* R38 item 3 / R39 item 1a: the grid's first row begins
       GRID_START_GAP_PX below the toggle group's bottom — derived from
       the group's placed box, so the two rulings (the toggle's anchor,
       the grid's start) stay one chain. The grid reads it as its rows'
       padding-top. */
