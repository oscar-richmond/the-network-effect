...
await p.goto(BASE + (PAGE === 'founders' ? '/founders?splash=0&forcehover' : PAGE === 'work' ? (FOCUS === 'switch' ? '/work?splash=0&forcehover' : '/work?view=list&splash=0&forcehover') : PAGE === 'services' ? '/services?splash=0&forcehover' : PAGE === 'case' ? `/work/${CASE}?splash=0&forcehover` : PAGE === 'modal' ? `${ROUTE}${ROUTE.includes('?') ? '&' : '?'}splash=0&forcehover` : '/?splash=0&forcehover'), { waitUntil: 'networkidle', timeout: 90000 }); await p.waitForTimeout(3500);
...
if (PAGE === 'work' && FOCUS === 'switch') {
  ...
  const WSTATE = `(() => { const st = getComputedStyle(document.querySelector('.work-stage')); const hdr = document.querySelector('[data-work-header]'); const hcs = hdr ? getComputedStyle(hdr) : null; const H = (el) => { const r = el.getBoundingClientRect(); const cs = getComputedStyle(el); /* document-relative: the header scrolls with the page by design (the switch scrolls to top) — what must not change is its place in the document, its width, opacity, filter */ return [Math.round(r.left * 10) / 10, Math.round((r.top + scrollY) * 10) / 10, Math.round(r.width * 10) / 10, +cs.opacity, cs.filter, cs.visibility].join('|'); }; const hdrSig = hdr ? [H(hdr), ...[...hdr.querySelectorAll('.work-page__hl')].map(H), ...[...document.querySelectorAll('[data-work-viewtoggle] button')].map(H)].join(' ; ') : null; return { hdrSig, view: window.__workView?.view(), switching: !!window.__workView?.switching(), pressed: [...document.querySelectorAll('[data-work-view]')].map((b) => b.dataset.workView + ':' + b.getAttribute('aria-pressed')).join(' '), lc: window.__lc ? window.__lc.add - window.__lc.remove : null, cursors: window.__viewCaseCursorCount || 0, listHandle: !!window.__workPage, gridHandle: !!window.__workGrid, lenis: document.documentElement.classList.contains('lenis'), stageDisplay: st.display, bodyOverflow: getComputedStyle(document.body).overflow, st: (window.gsap && window.ScrollTrigger) ? ScrollTrigger.getAll().length : (window.__workPage?.gsap?.globals?.().ScrollTrigger?.getAll?.().length ?? null), scrollY: Math.round(scrollY), gridVisible: [...document.querySelectorAll('[data-work-gtile]')].filter((t) => t.classList.contains('is-visible')).length }; })()`;
  ...
  const check = (st, tag) => { wchecks += 1;
    ...
    /* order-agnostic (R38 put GRID first in the markup): the active view's chip is pressed, the other is not */
    const pressedMap = Object.fromEntries((st.pressed || '').split(' ').map((kv) => kv.split(':')));
    if (pressedMap[v] !== 'true' || pressedMap[v === 'grid' ? 'list' : 'grid'] !== 'false') viol.push({ inv: 'W5', tag, view: v, msg: 'aria-pressed mismatch', pressed: st.pressed });
...
if (PAGE === 'modal') {
  ...
  for (let i = 0; i < MOVES; i++) {
    ...
    if (rnd() < 0.25 && (await f().evaluate(() => !!document.querySelector('[data-ct-open]')))) { await f().evaluate(() => document.querySelector('[data-ct-open]').click()); await settle(900); const o2 = await f().evaluate(MST); mchecks += 1; if (o2.open || !o2.ct) viol.push({ inv: 'M4', move: i, msg: 'both modals open', o2 }); await f().evaluate((i2) => document.querySelector(`[data-sp-qa="${i2}"]`).click(), t.i); await settle(900); const o3 = await f().evaluate(MST); mchecks += 1; if (!o3.open || o3.ct) viol.push({ inv: 'M4', move: i, msg: 'schedule modal did not yield', o3 }); }
