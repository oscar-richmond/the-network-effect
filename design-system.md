...
| Hero twirl (`hero.js`) | The image window turns `--m-hero-twirl-turns` about Y under `--m-hero-perspective` as it rises past the copy (the desktop cards' turn), scrubbed. | 1 turn over the rise, eased 1.4 |
| Band pin + exit (`band.js`, `band.css`) — WHO WE ARE, OUR NETWORK | The desktop founders mechanic: the section is sticky in a track that is its height + `--m-band-hold` + `--m-band-exit` + `--m-band-release` (+ `after`); the pin is at the top or on the foot where taller than the viewport, but never with the first ink under the bar (`--m-band-ink`, measured). Items drift in (`--m-drift-*`, `--m-drift-scale`) to land at the pin, hold, then exit at their own pace — `--m-exit-y-*` and `--m-exit-blur-*` per tier with the media a lag behind (`--m-exit-media-lag`), fading to 0; the network's edge gradients follow via `--m-band-y` / `--m-band-fade`. The network's `after` is the fade runway plus what the pinned stage leaves bare, so the grey→white fade begins at the release, never mid-exit. | founders pin −38 (label at 68) · network pin −150 (subtitle at 68) · hold 250 · exit 900 · release 760 |
...
| Rail pin (`rail-pin.js`) — FEATURED WORK, MOST BRANDS | The stage is sticky with its indicator `--m-rail-pin-bottom` above the viewport's bottom (floored at `--m-rail-pin-floor`); a runway after it is the rail's travel + `--m-rail-hold`; the page's scroll drives `scrollLeft` 1:1 over the travel (snap off while driven, touch still works). The last card ends at the inset. | pin −34 / −49 · indicator at 794 · featured runway 1723 · brands runway 974 |
| We Create Access (`access.js`, `access.css`) | The rows pin with their foot 80 above the viewport's bottom (`--m-access-pin-bottom`); the statement is sticky 80 above them (`--m-access-st-top`, its height measured into `--m-access-st-h`) and holds through the runway; at the release the rows scale to `--m-access-exit-scale`, lift `--m-access-exit-lift` and fade, the statement blurs and fades (`--m-access-exit-blur`). The statement's 438 + 80 + the rows' 496 exceed the viewport under the bar, so its top lines are above the viewport while pinned — a fact of the frame's sizes, not a defect. | pin 298 · lines' foot 218 |
| Closing red | `--m-closing-pin-top` is `min()`-capped so a red runway always survives below the text when the footer arrives. | 105 of red at 750; footer top 149, bottom 92 |

The harness's N2 now measures the wordmark's RENDERED size (font-size × the link's scale). Deviations, all reported: the hero's twirl turns the image window itself (the desktop turns three cards); the pillars' body copy does not reveal (by brief); the access statement's top lines sit above the viewport while pinned (see the table); on a 750-tall viewport the network's strip pins at the viewport's foot and reveals as the drift completes.
