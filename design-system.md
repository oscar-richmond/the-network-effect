...
- **The wordmark's scale** (`nav.js`): 24 → 18 scrubbed over `--m-nav-shrink-scroll` (120px) from the top, `ease: 'none'`, reversible. Since the motion pass (§4.13) the scrub is a `transform: scale` on the link (a composited layer, origin at its left edge) between the two TRUE type sizes, which are set at the rest states — a `font-size` scrub re-rasterises the glyphs at every step and reads as a vibration. The wordmark and the burger are each their own difference-blend host (iOS Safari blends a fixed bar against the root, not the page — the bar itself is `normal`), and `ground.js` paints the ground on `body` as well as the layer so the blend always has a backdrop.
- **The grounds** (`ground.js`): one fixed page-level layer (`.m-ground`, `z-index: var(--m-z-ground)`) carries the page colour, mirrored onto `body`; every ground-bearing section is transparent and declares `data-ground="light|dark|red"`. **Nothing draws a gradient**: a transition is the ground itself fading over `--m-ground-fade-px` (474). `data-ground-fade` runs the fade so it ENDS as the section's top reaches the viewport's bottom (the arriving band is already on its ground); `data-ground-fade-from="<selector>"` starts it at a named tail's top (the `.m-tail` after an opaque stage: services → Featured, Featured → Access); `data-ground-fade-anchor="pin"` runs it from a sticky pin (the closing red). Without a fade the switch is a hard edge at the section's top crossing the viewport centre. Sampled at every 40px of scroll on `/`: the fades step at most 1 per channel per pixel.
...
| Closing statement | Fourteen 48/45 lines at the frame's indents (four right-aligned; VALUE 32 in). The stage is sticky at `--m-closing-pin-top` (centred: the bar's foot + half the remaining height, 156 at 874) with 600 of runway; the ground layer AND the section (`data-ground-paint`) scrub grey → red from the pin ("pin" anchor, `--m-closing-red-px`); the bar goes solid ink as the fade begins (on the light ground the blend's own result is one step from the solid) and back to the blend as the footer's top crosses its midpoint. The red ends at the section's edge: the footer paints its own ground — a cut, as 1:372 is drawn. | pin 156 · fade 600 · 85 of red below the text after release |
...
### 4.13 The landing's motion pass (2026-09-08, the go-live fixes)

The previous build's entrances, pins and exits, brought onto the Part 2–3 layout as three generic modules plus per-section wiring. Every scrubbed motion stays a pure function of scroll; the entrances are the one exception (an entrance is an event — `once`, as on the desktop). Reduced motion turns all of it off: no wrap, no pin, no drift.

| Module | Mechanism | Numbers at 402×874 |
|---|---|---|
| Text reveal (`reveal.js`, `reveal.css`) | Every word of a block is wrapped in a clip (an inline-block window the word's own size, `--m-reveal-clip-room` / `--m-reveal-clip-rise` of room with compensating margins, so the line boxes are byte-identical) whose inner rises from `--m-reveal-park` on `--m-reveal-curve` over `--m-reveal-dur`; delays are staged from the RENDERED lines (`--m-reveal-line-stagger` per line, `--m-reveal-word-stagger` per word). Plays once as the block's top reaches `--m-reveal-at` of the viewport; the clips release their overflow when the last word lands. Every block of copy below the seam, the hero's (on the splash's hand-off) and the footer's — not the stacked pillars' content. | dur 1.2s · line 0.12s · word 0.04s · at 85% |
| Media reveal (`bindMediaReveal`) | A fade-rise `--m-reveal-media-rise` → 0 over `--m-reveal-media-dur`, a group staggered `--m-reveal-media-stagger`, a beat behind its text. The hero image, the founders' CTAs and cards, the strip's windows, the featured and brands cards, the access cells. | rise 24 · 0.8s · delay 0.3s · stagger 0.1s |
| Hero turn (`hero.js` + the desktop's `hero-rotating-gallery.js`) | The desktop's own WebGL gallery, run with one card: once the image has landed on its fade-rise, a plane mirrors the window's rect every frame in a fixed viewport layer (`.m-hero__gl`) and the DOM image is hidden; as the window rises off the top the plane makes its full turn about the vertical axis with the desktop's per-vertex fold — the same shader, the same easing, nothing re-guessed. The plane pauses once the window has cleared the top. The crop is centred (`--m-hero-img-pos` 50% 50%) so the takeover is seamless. Reduced motion keeps the static DOM image. | the desktop's turn, 1:1 |
| Band pin + exit (`band.js`, `band.css`) — WHO WE ARE, OUR NETWORK | The desktop founders mechanic: the section is sticky in a track that is its height + `--m-band-hold` + `--m-band-exit` + `--m-band-release` (+ `after`); the pin is at the top or on the foot where taller than the viewport, but never with the first ink under the bar (`--m-band-ink`, measured). Items drift in (`--m-drift-*`, `--m-drift-scale`) to land at the pin, hold, then exit at their own pace — `--m-exit-y-*` and `--m-exit-blur-*` per tier with the media a lag behind (`--m-exit-media-lag`), fading to 0 (a gone item takes no taps); the network's edge gradients follow via `--m-band-y` / `--m-band-fade`. **The handoff** (the desktop's: the network pulled a viewport up under the founders, revealing as their photo passes): the network section is pulled up by `--m-band-overlap` (measured — `h_f + release − T_net + T_f`) so its stage is PINNED beneath the founders exactly as their hold ends; it holds through their release (`holdExtra`), its copy reveals as their headline has gone (`bandTimes`, +`--m-exit-blur-headline`), its strip as their rail has, nothing drifting; its own exit then runs with `after` 0, so the outro's grey→white fade runs through the exit's last stretch and FROM ACCESS follows on the frame's 229. The founders track paints above (`--m-z-band-over`). | founders pin −38 (label at 68) · network pin −150 (subtitle at 68) · hold 250 · exit 900 · release 760 · network pinned at founders pin + 250 · its hold 1010 |
| Services stack (`services.js`) | WHAT WE DO is sticky at `--m-wwd-pin-top` (24 under the scrolled wordmark) and its box, the first pillar's negative margin and every pillar's `min-height` are equalised to `--m-stack-h` (measured) so the whole stack releases together, AMPLIFY first. Titles compact by `transform: scale` at origin 0 0 (no font-size scrub). As pillar i+1 rolls over pillar i, i's image, description, label, list and button dissolve one by one (opacity + `--m-wipe-blur` over `--m-wipe-span`, led by `--m-wipe-lead`). The transparent tail after the stage is where the ground fades to dark. | pins 67 / 97 / 128 · release −51 |
| Rail pin (`rail-pin.js`) — FEATURED WORK, MOST BRANDS | The stage is sticky with the FOOT of its indicator `--m-rail-pin-bottom` (64) above the viewport's bottom (floored at `--m-rail-pin-floor`); a runway after it is the rail's travel + `--m-rail-hold`; the page's scroll drives `scrollLeft` 1:1 over the travel (snap off while driven, touch still works). The last card ends at the inset. **Featured's fade-to-light** (the desktop's R6 item 2): the stage is transparent, the ground fades dark → light behind the departing stage from the tail's top, and the light ink — the title, the card titles and descriptions, the indicator (`--m-ind-op`) — fades out on the same window, so no light text sits over a shifting ground. | pin −18 / −33 · indicator's foot at 810 · featured runway 1723 · brands runway 974 |
| We Create Access (`access.js`, `access.css`) | The rows pin with their foot 64 above the viewport's bottom (`--m-access-pin-bottom`); the statement is sticky 80 above them (`--m-access-st-top`, its height measured into `--m-access-st-h`) and holds through the runway; at the release the rows scale to `--m-access-exit-scale`, lift `--m-access-exit-lift` and fade, the statement blurs and fades (`--m-access-exit-blur`). The statement's 438 + 80 + the rows' 496 exceed the viewport under the bar, so its top lines are above the viewport while pinned — a fact of the frame's sizes, not a defect. | pin 298 · lines' foot 218 |
| Closing red | The statement is sticky at `--m-closing-pin-top`, centred between the bar's foot and the viewport's bottom (the desktop rule — the `min()` cap that moved it on short viewports is gone, Oscar 2026-09-08). The red under VALUE as the footer arrives is the band's 85 (`--m-closing-st-bottom`, the section's padding: the sticky releases on it) at every height — the footer never comes over it. The footer reads `--m-footer-top` / `--m-footer-bottom` (the frame's 229 less 80; 44 plus 48). | pin 156 at 874, 94 at 750 · 85 of red at the footer · footer 149 / 92 |

**The bar's ink** is computed, not blended (the third report of an invisible wordmark on the phone): `mix-blend-mode: difference` on a fixed bar is not reliable on iOS — it composites against the root layer and the wordmark rendered plain white over the light ground. `ground.js` writes `--m-nav-ink` on `:root` as the INVERSE of the ground the layer shows (what the blend gives against a bare ground), resynced on every fade update and every ScrollTrigger refresh (refresh renders the scrubbed fades with events suppressed); the wordmark and the burger take it as a plain colour, solid ink over the red band, white with the menu open. Declared at rest in `colour.css` as the ink.

The harness's N2 measures the wordmark's RENDERED size (font-size × the link's scale); N4 checks the computed ink against the layer's colour at every step. Deviations, all reported: the hero's turn runs the desktop gallery with one card (the desktop turns three); the pillars' body copy does not reveal (by brief); the access statement's top lines sit above the viewport while pinned (see the table); on a 750-tall viewport the network's strip pins at the viewport's foot.
...
| Footer | `--m-footer-top`, `--m-footer-bottom`, `--m-footer-indent`, `--m-footer-statement-h`, `--m-footer-index-pitch`, `--m-footer-link-h`, `--m-footer-legal-row-pitch` | 149 (the frame's 229 less 80), 92 (44 plus 48), 104, 180, 30, 18, 40 | 1:409, 1:393 – 1:396, 1:410 |
...
| Splash / colour | `--m-z-splash`, `--m-z-band-over`, `--m-color-muted`, `--m-nav-ink` | 11000, 2, #6c6c6e (ruled), the ink at rest (ground.js rewrites it as the ground's inverse) | — |
| Motion pass | `--m-rail-pin-bottom`, `--m-access-pin-bottom`, `--m-band-overlap`, `--m-ind-op` | 64, 64, measured (network.js), 1 (featured.js drains it) | Oscar 2026-09-08 |
